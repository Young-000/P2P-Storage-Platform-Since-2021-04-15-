...
Despite the rapid growth of public cloud services like Amazon Web Services (AWS), the demand for data storage continues to outpace supply. According to recent industry reports, the global cloud storage market size is projected to grow from USD 108 billion in 2023 to over USD 250 billion by 2028 \citep{mordor2024cloud}. Simultaneously, the decentralized storage market has gained significant traction. As of 2024, platforms like Filecoin have accumulated over 1.5 EiB (Exbibytes) of raw storage capacity, and Storj hosts petabytes of data across tens of thousands of active nodes globally \citep{filecoin2024status, storj2024report}. This surge highlights the viability of peer-to-peer (P2P) storage as a complement to centralized cloud services. However, unlike centralized providers with elastic capacity, P2P platforms rely on the voluntary participation of independent providers, creating a unique challenge: \textit{supply is endogenous and potentially scarce}.
...
\textcolor{red}{New Lemma-------------}
\begin{Lemma}\label{lem:pricing}
For a given pricing design characterized by the bandwidth allowance $q$, the platform's unconstrained profit-maximizing service fees $(p_s^o, p_b^o)$ are:
\begin{equation*}
    (p_s^o, p_b^o) = \begin{cases}
    (0, \frac{1}{2}) &\text{for the two-part tariff } (q=0),\\
    (\frac{q \lambda_0}{2\theta}, \frac{1}{2}) &\text{for a hybrid design with } q \in (0,1],\\
    (\frac{3q(2q^2 - 1)}{2(4q^3-1)} \frac{\lambda_0}{\theta}, \frac{3q^2(2q-1)}{2(4q^3-1)}) &\text{for a hybrid design with } q > 1,\\
    (\frac{3 \lambda_0}{4\theta}, -) &\text{for subscription-based pricing } (q \to \infty).
    \end{cases}
\end{equation*}
\end{Lemma}
----------------------
...
% ========================================================================
% [Insert after Theorem 1 in Section 4.2]
% ========================================================================
\noindent \textcolor{red}{\textbf{Intuition (The Separation Principle).} 
Theorem \ref{thm:redundancy} implies a separation principle: the technological decision of choosing redundancy parameters $(\theta, t)$ can be decoupled from the pricing decision. Since redundancy affects profit solely through the effective provider cost $\xi(t)\theta$, the platform should always minimize this aggregate operating cost, regardless of whether it employs a subscription or two-part tariff. This provides a clear managerial guideline: optimize engineering efficiency first, then optimize pricing strategy.}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Under Constrained Pricing}\label{subsec:constrained_surplus}

The consequences of pricing schemes under market-clearing prices are less straightforward because service fees interact with renter decisions through the binding supply level. Before comparing the schemes, we first establish an equivalence result that simplifies our analysis.

\textcolor{red}{\begin{prop}\label{prop:low_q_equivalence}
\textbf{(Equivalence of Low-Allowance Designs)}
Consider any hybrid pricing design with a small allowance $q \le \lambda_0$ (where $\lambda_0$ is the minimum usage). The platform's profit and total system surplus under this design are identical to those under the two-part tariff ($q=0$). Specifically, the optimal fixed fee adjusts to $p_s(q) = p_s(0) + \frac{q p_b}{\theta}$, neutralizing the effect of the free allowance.
\end{prop}}

\textcolor{red}{This proposition implies that in the low-allowance regime, the hybrid scheme offers no structural advantage over the two-part tariff. Thus, in the following theorem, we focus on comparing the two-part tariff and hybrid pricing with a large allowance ($q \ge 1$) against subscription pricing.}

\textcolor{red}{\begin{Theorem}\label{thm:surplus_market}
Suppose the provider capacity is scarce ($n_p < \psi$). The welfare ranking depends on the severity of the supply constraint:
\begin{enumerate}[label=(\roman*)]
    \item \textbf{(Moderate Scarcity)} In the intermediate regime ($\psi^T \le n_p \le \psi^S$), a hybrid design with a sufficiently large allowance ($q \ge 1$) yields higher total surplus than subscription pricing ($TS^{Hh} \ge TS^S$).
    \item \textbf{(Severe Scarcity)} In the highly constrained regime ($n_p \le \psi^T$), the two-part tariff yields higher total surplus than subscription pricing ($TS^T \ge TS^S$).
\end{enumerate} 
\end{Theorem}}

\textcolor{red}{\noindent \textbf{Intuition (Allocative Efficiency under Scarcity).} 
The superiority of the two-part tariff under scarcity is driven by allocative efficiency. Under subscription ($p_b=0$), the only way to clear the market is to raise the access fee $p_s$, which indiscriminately excludes users based on total valuation. In contrast, the two-part tariff ($p_b > 0$) introduces a marginal cost that acts as a screening mechanism, throttling excessive usage by heavy users without excluding light users. This `pricing of scarcity' allows the two-part tariff to sustain higher participation and total surplus.}
...
\textcolor{blue}{\textbf{현재 진행중}}

We conduct numerical experiments to validate our theoretical findings and examine the robustness of the results under broader conditions. Our primary objective is to verify the \textit{regime switch} in welfare ranking predicted by Theorems \ref{thm:surplus_first} and \ref{thm:surplus_market}, and to demonstrate that these insights hold across different demand distributions and cost structures.

\subsection{The Welfare Crossover: Supply Scarcity and Pricing Regimes}
We first examine how the total system surplus changes as the provider capacity varies from scarcity to abundance. Figure \ref{fig:regime_switch} illustrates the total surplus generated by the subscription ($S$), two-part tariff ($T$), and hybrid ($Hh$, with $q=2.0$) schemes as a function of the number of providers $n_p$.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{Figure5_Welfare_Crossover.png} 
    \caption{\textbf{The Welfare Crossover.} Under scarcity (low $n_p$), the two-part tariff maximizes surplus by efficiently rationing demand. As supply becomes abundant (high $n_p$), subscription pricing dominates by eliminating deadweight loss. Hybrid pricing serves as an effective intermediate design.}
    \label{fig:regime_switch}
\end{figure}

Consistent with our theoretical predictions, we observe a clear \textit{crossover} in performance:
\begin{itemize}
    \item \textbf{Scarcity Regime ($n_p$ Low):} The two-part tariff (red line) yields significantly higher surplus than subscription (blue line). Under tight supply, the subscription scheme ($p_b=0$) fails to throttle excessive usage, forcing the platform to raise the access fee $p_s$ to clear the market. This excludes many low-valuation users, reducing total welfare. In contrast, the two-part tariff uses the marginal price $p_b$ to suppress low-value consumption without excluding users, maintaining higher allocative efficiency. This confirms Theorem \ref{thm:surplus_market}.
    \item \textbf{Abundance Regime ($n_p$ High):} As provider capacity increases, the surplus under subscription rises steeply and eventually overtakes the two-part tariff. When capacity is ample, the marginal cost of serving an additional unit is negligible. Therefore, the optimal strategy is to set $p_b=0$ to eliminate consumption distortion. This aligns with Theorem \ref{thm:surplus_first}, validating that subscription dominates in the unconstrained benchmark.
    \item \textbf{Intermediate Regime:} The hybrid scheme (green line) serves as a bridge. By offering a free allowance ($q$), it mitigates the distortion for light users while still managing congestion for heavy users, performing well in the moderate scarcity range.
\end{itemize}

\subsection{Robustness and Reality Checks}
To ensure that our results are not driven by specific modeling assumptions, we performed extensive robustness checks.

\paragraph{Distribution Invariance.}
Critics might argue that the results depend on the heavy-tailed nature of the Pareto distribution. We tested the same scenarios under \textit{Lognormal} and \textit{Exponential} demand distributions. In all cases, the qualitative pattern of the regime switch remained identical: the two-part tariff dominates in scarcity, while subscription dominates in abundance. This suggests that the welfare ranking is driven by the fundamental economic trade-off between screening efficiency and usage distortion, not by the tail properties of a specific distribution.

\paragraph{Impact of Market Realities.}
We relaxed the assumption that renter utility depends solely on usage ($U = \lambda u$) by adding a fixed utility component for storage access ($U = K + \lambda u$). Numerical results show that while a higher $K$ shifts the total surplus curves upward (increasing participation), it does not alter the relative ranking of the pricing schemes or the location of the crossover point. Furthermore, we considered a scenario where providers require a fixed reservation payment ($f$) to participate. A higher $f$ effectively reduces the number of active providers, pushing the market towards the scarcity regime. Paradoxically, this structural constraint reinforces our main finding: as fixed costs tighten the effective supply, the efficiency gain from the two-part tariff (over subscription) becomes even more critical for the platform's sustainability.

\subsection{Validation of Pricing Design Approach}
Finally, we validated the structural relationship between the hybrid and two-part schemes. In the low-allowance regime ($q \le \lambda_{0}$), we confirmed that the profit and surplus of the hybrid scheme converge exactly to those of the two-part tariff (Difference $\approx 0.0000$). This numerical equivalence supports our theoretical framing in Proposition \ref{prop:low_q_equivalence}, justifying the treatment of $q$ as a discrete design parameter.
...
\paragraph{Proof of Proposition \ref{prop:low_q_equivalence}}
\begin{proof}
Consider a hybrid design with allowance $q \le \lambda_{0}$, where $\lambda_{0}$ is the minimum usage of active renters. The total payment for any renter is $T_{hybrid} = \theta p_s + (\lambda - q)p_b$. 
Let us define a transformed fixed fee $p_s' = p_s - \frac{q p_b}{\theta}$. 
Then, the payment becomes $T_{hybrid} = \theta p_s' + \lambda p_b$, which is algebraically identical to the payment under a two-part tariff with fees $(p_s', p_b)$.
Since the payment function faced by every user is identical, the renter demand, provider participation, and market-clearing conditions remain unchanged. 
Thus, the optimization problem is isomorphic, and the equilibrium outcomes (profit, surplus) are identical.
\end{proof}
